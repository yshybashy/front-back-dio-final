...
export default function ModalEdit({name}) {
  const {setOpenModalEdit, infoVideo, setAttPage, attPage} = useContext(Context);
  ...
  async function edit(){
    ...
    if(nameInput.value && descriptionInput.value && imgInput.value && (erroInputName===false) && (erroInputDescription===false) && (erroInputImage===false)){
      ...
      if(name == 'Filme'){
        await new FilmsApi().update('/netflix/films/update/', infoVideo.id, obj).then(res => {
          setOpen(false);
        ...
        await new SeriesApi().update('/netflix/series/update/', infoVideo.id, obj).then(res => {
          setOpen(false);
          setOpenModalEdit(false);
          setAttPage(!attPage);
        }).catch(err => {
  ...
  return(
    ...
    <Modal
    show={open}
    onHide={() => (setOpen(false), setOpenModalEdit(false))}
    >
    <Modal.Header style={{padding:'1.25rem 1.25rem 0 1.25rem'}}>
      <Modal.Title style={{color:'black'}}>Editar {name}
      </Modal.Title>
        <h5 
        style={{cursor:'pointer', color:'black'}}
        onClick={() => (setOpen(false), setOpenModalEdit(false))}
        >X</h5>
    </Modal.Header>
    <Modal.Body>
      <Row className='mb-3'>
      ...
      <Row className='mb-3'>
        <Col className="order-xl-1" xl="30">
          ...
          <Input
            defaultValue={infoVideo.description}
